...
class FriendsScreen extends StatefulWidget {
  ...
  static Route getFriendsRoute(RouteSettings settings) {
    ...
    return CupertinoPageRoute(builder: (context) => FriendsScreen(user: user));
  }
...
class _FriendsScreenState extends State<FriendsScreen> {
  ...
  Future<void> _fetchFriends() async {
    ...
    try {
      final newItems = await service.fetchFriendsForUser(widget.user.id);
      setState(() {
        _items
          ..clear()
          ..addAll(newItems);
      ...
      Logger().debug('Error fetching friends: $e');
    } finally {
  ...
  Future<void> _fetchData() async {
    ...
    try {
      final newItems = await service.fetchUsersFromQuery(_query);
      setState(() {
        _items
          ..clear()
          ..addAll(newItems);
    ...
    return CupertinoScrollbar(
      child: ListView.separated(
        ...
        itemBuilder: (context, index) {
          ...
          return ListTile(
            tileColor: lightGrayWithPurple,
            contentPadding: EdgeInsets.symmetric(horizontal: paddingSize / 2, vertical: 4),
            leading: Container(
              width: avatarSize,
              height: avatarSize,
              decoration: BoxDecoration(
